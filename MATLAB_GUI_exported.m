...
if length(str) > 0 %Vi går kun ind i dette if-statement, hvis der er manglende informationer/indtastninger
   warndlg(str,'Fejl');
   return; %Afbryder og går ikke videre herfra
end

%% Forbinder til Bluetooth
try
    my_BLE_Device = ble("ESP32"); %Forsøger at oprette Bluetooth-forbindelse til ESP32 
catch e
    disp(e.message);
return
end

%Indlæser service og characteristic fra ESP32
...
%Gør 'Bluetooth-lampen' grøn, hvis computeren er forbundet til ESP32
...
while(app.keep_running)
    pause(1); %Tjekker hvert sekund om keep_running har ændret sig (true/false).
end
    
%Læser og printer input fra ESP32
...
%AC-amplituden printes i tekstboksen og lampen gøres grøn
...
%Læser og gemmer indtastede informationer
alder = app.AlderEditField.Value;



%Algoritme (Vores egen), som enten runder op eller ned til nærmeste heltal
CRF = round(0.31644*AC_amplitude_mg+29.5092); 

%Vurdering af fysisk form på baggrund af køn og alder
   ...
   switch sex
       case 'Kvinde'
        if (alder>=20) && (alder<=29)
            if (CRF<24)
                app.FysiskformEditField.Value = 'Meget dårlig';
                app.Gauge.Value = CRF;
            elseif (CRF>=24) && (CRF<=30)
                app.FysiskformEditField.Value = 'Dårlig';
                app.Gauge.Value = CRF;
